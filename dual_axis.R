...
dual_axis_graph <- function(graph_1, graph_2, label_1, label_2)
{
  ...
  dual_grob2 <- gtable_add_rows(dual_grob, height, 2)
  dual_grob2 <- gtable_add_grob(dual_grob2, wrap_labels, t=3, l=3, r=5, z = 10)
  
  height <- unit(1, "grobheight", left_label)
  dual_grob2 <- gtable_add_rows(dual_grob2, height, 3)
  dual_grob2 <- gtable_add_grob(dual_grob2, rectGrob(gp=gpar(col="white"), height = height),
                                t=4, l=3, r=5, z = 2)
  
  grid.draw(dual_grob2)
}
